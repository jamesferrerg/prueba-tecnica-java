...
public class LibroController {
	...
	public ResponseEntity<?> mostrar(@PathVariable Long idLibro){
		Libro libro = null;
		...
		if(libro == null) {
			response.put("mensaje", "El libro con ID ".concat(idLibro.toString().concat(" no existe en la base de datos")));
	...
	public ResponseEntity<?> create(@Valid @RequestBody Libro libro, BindingResult result){
		...
		response.put("mensaje", "El libro ha sido creado con exito");
		response.put("libro", libroNew);
		return new ResponseEntity<Map<String, Object>>(response, HttpStatus.CREATED);
	}
	
	@PutMapping("/libros/{idLibro}")
	public ResponseEntity<?> update(@Valid @RequestBody Libro libro, BindingResult result, @PathVariable Long idLibro){
		
		Libro libroCurrent = libroService.findById(idLibro);
		...
		if (libroCurrent == null) {
			response.put("mensaje", "Error: no se puede editar el libro con id: ".concat(idLibro.toString().concat(" no existe en la base de datos")));
			return new ResponseEntity<Map<String, Object>>(response, HttpStatus.NOT_FOUND);
		}
		
		try {
			libroCurrent.setTitulo(libro.getTitulo());
			libroCurrent.setFechaYear(libro.getFechaYear());
			libroCurrent.setGenero(libro.getGenero());
			libroCurrent.setNumeroPaginas(libro.getNumeroPaginas());
			libroCurrent.setEditorial(libro.getEditorial());
			libroCurrent.setAutor(libro.getAutor());
			
			libroUpdate = libroService.save(libroCurrent);
